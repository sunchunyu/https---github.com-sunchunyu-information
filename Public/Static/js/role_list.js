$(function () {
    ...
    $(".btn-edit").on("click", function () {
        ...
        if (id === undefined) {
            bootMessage("warning", "亲，请先选择要编辑的角色~~~")
        } else {
    ...
    $(".btn-power").on("click", function () {
        var id = $(".tr-selected").attr("data");
        if (id === undefined) {
            bootMessage("warning", "亲，请先选择要分配权限的角色~~~")
        } else {
            window.location.href = $(this).attr("data") + "?id=" + id;
        }
    });
    $(".btn-del").on("click", function () {
        var ids = $(".tr-selected");
        if (ids.length == 0) {
            bootMessage("warning", "亲，请先选择要删除的角色~~~")
        } else {
            bootConfirm("亲，您确定要删除这个角色吗？", function () {
                var arr = [];
...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            ...
            $(nRow).find("td:last").html("<a class='btn btn-info btn-xs' data='" + aData.Id + "' onclick='power(this,event)'><i class='glyphicon glyphicon-ok'></i> 权限 </a>&nbsp;<a class='btn btn-primary btn-xs' data='" + aData.Id + "' onclick='eidt(this,event)'><i class='glyphicon glyphicon-pencil'></i> 编辑 </a>&nbsp;<a class='btn btn-danger btn-xs' data='" + aData.Id + "' onclick='del(this,event)'><i class='glyphicon glyphicon-trash'></i> 删除 </a>");
