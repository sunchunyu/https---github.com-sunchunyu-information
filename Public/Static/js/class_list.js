$(function () {
    ...
    $(".btn-edit").on("click", function () {
        ...
        if (id === undefined) {
            bootMessage("warning", "亲，请先选择要编辑的记录~~~")
        } else {
    ...
    $(".btn-del").on("click", function () {
        ...
        if (ids.length === 0) {
            ...
            bootConfirm("亲，您确定要删除这个班级吗？", function () {
                var arr = [];
...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        "columns": [
            {"data": "name"},
            {"data": "specialty_id.name"},
            {"data": "specialty_id.college.name"},
            {"data": "specialty_id.college.college"},
